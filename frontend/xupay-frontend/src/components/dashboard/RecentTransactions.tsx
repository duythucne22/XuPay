...
export function RecentTransactions({ userId, limit = 5, onTransactionClick }: RecentTransactionsProps) {
  const { data, isLoading, isError } = useTransactions({ userId, page: 1, size: limit }) as any
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-xs ${
                tx.type === 'received' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-white/5 text-gray-300'
              }`}>
            ...
            <div className="text-right">
              <p className={`text-sm font-medium ${
                ...
                {(tx.type === 'received' ? '+' : '-')}${(tx.amountCents / 100).toFixed(2)}
              </p>
              <p className="text-xs text-gray-500">{tx.type === 'received' ? 'Received' : 'Sent'}</p>
            </div>
          </div>
        ))}
      </div>
